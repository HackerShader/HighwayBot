...
await console.log(color.code.yellow,'[Notification] Getting info from github api...');
await superagent.get('https://api.github.com/repos/HackerShader/HighwayBot/releases/latest')
    ...
    .end(async (err, res) => {
        await console.log(color.code.yellow, '[Notification] Downloading resource...');
        ...
        await superagent.get(json.zipball_url)
            ...
            .end(async (err, res) => {
                ...
                await console.log(color.code.green,'[Done] Downloaded resource');
                await require('./unzip');
            });
    });
}
