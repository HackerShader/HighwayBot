...
module.exports = (args) => {
    if (!args[2])
        return console.log(color.code.blue, `[Config | Load] Usage: config load <filename>`);
    if (!fs.existsSync(`./config/${args[2]}.json`))
        return console.log(color.code.red, `[Config | Load | Error] Config [${args[2]}] does not exist`);
